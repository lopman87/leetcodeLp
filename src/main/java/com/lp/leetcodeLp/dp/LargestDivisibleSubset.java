...
public class LargestDivisibleSubset {
    ...
    public List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            for (int j = i+1; j < nums.length; j++) {
                if (compute(result, nums[j])){
                    result.add(nums[j]);
                }
            }
            finalResult.add(result);
        }
        java.util.Collections.sort(finalResult, new java.util.Comparator<List<Integer>>() {
            @Override
    ...
    public boolean compute(List<Integer> result , int target) {
