...
public class LargestDivisibleSubset {
    ...
    public List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            for (int j = 0; j < i; j++) {
                if (nums[i] % nums[j] == 0){
                    List<Integer> jj =  finalResult.get(nums[j]);
                    if (jj.size() > size){
                        size = jj.size();
                        flagIndex = nums[j];
                    }
            ...
            if (flagIndex!=0)result.addAll(finalResult.get(flagIndex));
            finalResult.put(nums[i],result);
        }
        return finalResult.values().stream().max(Comparator.comparing( List::size)).get();
    }
